                ...
                if selected_active is not None:
                    ...
                    if both_active.any():
                        ...
                        for seg_start, seg_end in zip(seg_starts, seg_ends):
                            ...
                            fig.add_shape(type="rect", x0=seg_t_start, x1=seg_t_end, y0=y_position - bar_height/2, y1=y_position + bar_height/2, fillcolor=fillcolor_full, line=dict(width=0, color='rgba(0,0,0,0)'))
                    if only_this_active.any():
                        ...
                        for seg_start, seg_end in zip(seg_starts, seg_ends):
                            seg_t_start = float(t_values[start_idx + seg_start]) if hasattr(t_values[start_idx + seg_start], 'item') else t_values[start_idx + seg_start]
                            seg_t_end = float(t_values[start_idx + seg_end - 1]) + 1 if hasattr(t_values[start_idx + seg_end - 1], 'item') else t_values[start_idx + seg_end - 1] + 1
                            fig.add_shape(type="rect", x0=seg_t_start, x1=seg_t_end, y0=y_position - bar_height/2, y1=y_position + bar_height/2, fillcolor=fillcolor_partial, line=dict(width=0, color='rgba(0,0,0,0)'))
                else:
                    fig.add_shape(type="rect", x0=t_start, x1=t_end, y0=y_position - bar_height/2, y1=y_position + bar_height/2, fillcolor=fillcolor_full, line=dict(width=0, color='rgba(0,0,0,0)'))
